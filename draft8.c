...
void find1(){
    ...
    char * in_file = (char *) malloc (100000 * sizeof(char));

    int now = 0;
    int found = 0;

    while(fgets(in_file, 100000, filepointer) != NULL){

        char * pos = strstr(in_file, given_str);
        if (pos){
            printf("%d\n", pos-in_file+now);
            found = 1;
            break;
